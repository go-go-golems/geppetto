...
- [x] Route tool-call execution result blocks back into Responses input
...
- [x] Emit tool-call events for Responses during streaming for each function call (complete across multiple parallel calls)
...
- [x] Document new `EventTypePartialThinking` and ensure printers support it (they do)

### Tooling
- [ ] Harden `PrepareToolsForResponses` against complex JSON Schema (nested refs, enums, defaults)
- [x] Support parallel tool calls in Responses: concurrency hints, iterative loop
...
- [x] Enforce omission of `temperature` and `top_p` for o3/o4 in all code paths
...
- [x] Add developer guide: “How Geppetto handles Responses SSE”
- [x] Include troubleshooting section for common 400 errors (tool_choice invalid, params not supported)
