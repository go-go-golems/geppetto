...
func (e *ClaudeEngine) RunInference(
	...
	if clientSettings == nil {
		return conversation.InferenceContext{}, steps.ErrMissingClientSettings
	}
	anthropicSettings := e.settings.Claude
	if anthropicSettings == nil {
		return conversation.InferenceContext{}, errors.New("no claude settings")
	}
	...
	if !ok {
		return conversation.InferenceContext{}, errors.Errorf("no API key for %s", apiType)
	}
	baseURL, ok := apiSettings.BaseUrls[string(apiType)+"-base-url"]
	if !ok {
		return conversation.InferenceContext{}, errors.Errorf("no base URL for %s", apiType)
	}
