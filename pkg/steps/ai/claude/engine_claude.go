	...
	for {
		...
		case event, ok := <-eventCh:
			...
			for _, event_ := range events_ {
				// Keep local metadata in sync with merger-derived metadata so we can
				// persist canonical inference metadata onto the returned turn.
				evMeta := event_.Metadata()
				if evMeta.StopReason != nil && strings.TrimSpace(*evMeta.StopReason) != "" {
					sr := strings.TrimSpace(*evMeta.StopReason)
					metadata.StopReason = &sr
				}
				if evMeta.Usage != nil {
					metadata.Usage = evMeta.Usage
				}
				if evMeta.DurationMs != nil {
					metadata.DurationMs = evMeta.DurationMs
				}
