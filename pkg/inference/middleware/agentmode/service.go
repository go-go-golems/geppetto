...
func (s *StaticService) GetMode(ctx context.Context, name string) (*AgentMode, error) {
    if name == "" { return nil, ErrUnknownMode }
    if m, ok := s.modes[strings.ToLower(name)]; ok { return m, nil }
...
func (s *SQLiteService) GetCurrentMode(ctx context.Context, runID string) (string, error) { return s.s.GetCurrentMode(ctx, runID) }
func (s *SQLiteService) RecordModeChange(ctx context.Context, change ModeChange) error { return s.s.RecordModeChange(ctx, change) }
